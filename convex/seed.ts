...
import { DatabaseWriter, mutation, MutationCtx } from "./_generated/server";
...
type User = Doc<"users"> & {_creationTime: number};

type Expense = Doc<"expenses"> & {_creationTime: number};

type Group = Doc<"groups"> & {_creationTime: number};
...
async function createGroups(ctx : Ctx, users : User[]) {
  const now = Date.now();
...
async function createOneOnOneExpenses(ctx : Ctx, users : User[]) {
  const now = Date.now();
  ...
  return await Promise.all(
    expenseIds.map(async (id) => {
      ...
      return { ...expense, _id: id } as Expense;
    })
  );
}

// Helper to create group expenses
async function createGroupExpenses(ctx : Ctx, users : User[], groups: Group[]) {
  const now = Date.now();
  const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000;
  const twoWeeksAgo = now - 14 * 24 * 60 * 60 * 1000;
